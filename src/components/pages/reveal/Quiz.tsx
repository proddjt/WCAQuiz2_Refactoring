...
export default function Quiz({mode, difficulty} : {mode: string, difficulty: string}){
    ...
    return (
        ...
        <Stack flex={1} hiddenFrom="md" px={"xl"} pb={"md"} pt={0} align="center" gap={30}>
            ...
            <Title order={1} mt={20} ta={"center"}>{attempts === 10 || gameOver ? person?.name : "*****************"}</Title>
            ...
            <Card title={t("med_and_rec")} Icon={FaMedal} h={"330px"} animation={attempts === 3 || attempts === 4 ? "new-clue" : ""} key={`TABLE-${attempts}`}>
                <Text fw={700} fz={"sm"}>{t("medals")}</Text>
                <TableWithCols items={[person?.medals]} condition={attempts < 3 && !gameOver} tableType="medals"/>
                <Text fw={700} fz={"sm"}>{t("records")}</Text>
                <TableWithCols items={[person?.records]} condition={attempts < 4 && !gameOver} tableType="records"/>
            </Card>

            <Card title={t("results")} Icon={RiTimerFill} h={"400px"} animation={attempts === 5 || attempts === 8 || attempts === 9 ? "new-clue" : ""} key={`TABLE-2-${attempts}`}>
                <TableWithCols
        ...
        >
            ...
            <Grid.Col span={1}>
                <ScrollArea h={maxHeight} type="hover" offsetScrollbars styles={{content: {display: "flex", flexDirection: "column", gap: 10}}}>
                    <Title order={1} mt={20} ta={"center"}>{attempts === 10 || gameOver ? person?.name : "*****************"}</Title>
