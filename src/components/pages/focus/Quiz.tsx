    ...
    <>
      <Stack flex={1} hiddenFrom="md" px={"xl"} pb={"md"} pt={0} align="center" gap={30}>
        ...
        <Title order={1} mt={20} ta={"center"}>
          {attempts === 5 || gameOver ? person?.name : "*****************"}
        </Title>

        <Card
          title={t("personal_info")}
          Icon={RiContactsFill}
          assignRef={assignRef([1, 4])}
        >
          <Group>
            <TextInput
              label={t("nation")}
        ...
        <Card title={t("comp_info")} Icon={IoMdPodium} assignRef={assignRef([2])}>
          <Group>
            <TextInput
              label={t("comp_numb")}
        ...
        <Card title={t("best_result")} Icon={IoIosStopwatch} h={"200px"} assignRef={assignRef([3])}>
          <TableWithCols items={person?.personal_records || []} condition={attempts < 3 && !gameOver} tableType="focus" />
        </Card>
      </Stack>
      ...
      >
        <Grid.Col span={1}>
          ...
          <Actions openInfo={focusModal}>
            ...
            <Button size="xs" fullWidth disabled={!gameOver} onClick={() => confirmationModal(t("start_new_modal_desc"), startNew)}>
              {t("start_new")}
            </Button>
          </Actions>
        </Grid.Col>
